...
執行流程：
  Phase 1（預設）：自動偵測基礎分支，多執行緒收集差異 diff → code-review/references/diff/*_diff.diff
  Phase 2（--report）：讀取 code-review/references/analysis/*_result.json → code-review/references/report/ 後清除 code-review/references/diff/
"""
...
CR_DIFF_DIR = "code-review/references/diff"          # 執行緒儲存 .diff 的目錄
CR_ANALYSIS_DIR = "code-review/references/analysis"  # AI 代理寫入 JSON 結果的目錄
CR_REPORT_DIR = "code-review/references/report"      # 彙整報告輸出目錄
...
def cleanup_diff_dir(diff_dir: str) -> None:
    ...
    print(f"[INFO] 正在清理暫存 diff 檔案於 {diff_dir}/ ...")
    if not diff_path.exists():
        print(f"[INFO] diff 目錄 {diff_dir}/ 不存在，無需清理")
        return
    deleted = 0
...
def report_phase(
    ...
    if not result_files:
        ...
        print("[HINT] 請先讓 AI 代理分析 code-review/references/diff/*_diff.diff 並將結果寫入 code-review/references/analysis/*_result.json，再執行 --report。")
        return None
    ...
    # 從 repo_path 提取專案名稱作為報告檔名
    project_name = Path(repo_path).name
    output_path = Path(report_dir) / f"{project_name}_report.md"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Code Review 主程式\n"
            "  不帶 --report：Phase 1 — 偵測基礎分支，多執行緒收集 diff → code-review/references/diff/*_diff.diff\n"
            "  帶 --report  ：Phase 2 — 讀取 code-review/references/analysis/*_result.json，輸出報告並清除 code-review/references/diff/"
        ),
